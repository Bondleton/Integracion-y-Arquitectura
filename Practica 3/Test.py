...
ventana = tk.Tk()
ventana.title("Secuencia de Semáforo con Parpadeo Verde")
...
# Labels para los focos
labels = {}
for i, color in enumerate(secuencia):
    labels[color] = tk.Label(frame_imagenes, image=imagenes_apagadas[color], bg=COLORES["fondo"])
    labels[color].grid(row=0, column=i, padx=20)
